...
private:
	...
	void find_possible_next(string &a, unordered_set<string>& dict, vector<string>& out)
	{
		...
		for (int i = 0; i < a.size(); i++)
		{
			...
			{
				new_word[i] = 'a' + j;
				if (dict.find(new_word) != dict.end() && new_word != a) out.push_back(new_word);
			}
		}
	}
public:
	vector<vector<string>> output(string start, string end, unordered_map<string, vector<string>> &cache)
	{
		...
		if (cache.find(start) == cache.end()) return res;
		auto next_list = cache[start];
		for (auto next_iter: next_list)
		{
			auto next_res = output(next_iter, end, cache);
			if (next_res.empty() == false)
			{
				//cout << "append " << start << endl;
				for (int i = 0; i < next_res.size(); i++)
				{
					next_res[i].insert(next_res[i].begin(), start);
				}
				res.insert(res.begin(), next_res.begin(), next_res.end());
			}
		}
		return res;
	}
    ...
    {
    	unordered_set<string> current_lvl, next_lvl;
       	current_lvl.insert(beginWord);
       	unordered_map<string, vector<string>> output_cache;
       	bool found = false;
		       
		//for (auto left: wordList)
       	//{
       	//	cout << left << " ";
       	//}
       	//cout << " LEFT" << endl;
       	wordList.insert(endWord);
       	while (current_lvl.empty() == false)
       	{
       		for (auto str_iter:current_lvl)
       		{
       			//cout << "current " << str_iter << endl;
       			vector<string> out;
       			find_possible_next(str_iter, wordList, out);
       			if (out.empty() == false)
       			{
       				for (auto next_str:out)
       				{
       					if (current_lvl.find(next_str) != current_lvl.end()) continue;
       					//cout << "\tnext " << next_str << endl;
       					output_cache[str_iter].push_back(next_str);
       					next_lvl.insert(next_str);
       				}
       			}
       		}
       		if (next_lvl.find(endWord) != next_lvl.end())
       		{
       			// Found it in next level!
       			//cout << "Found " << endWord << endl;
       			found = true;
       			break;
       		}
       		else
       		{
       			for (auto str_iter: current_lvl)
       			{
       				//cout << "\tErase " << str_iter << endl;
       				wordList.erase(str_iter);
       			}
       			for (auto left: wordList)
       			{
       				//cout << left << " ";
    	...
    	assert(output_cache.find(beginWord) != output_cache.end());
    	return output(beginWord, endWord, output_cache);
    }
};
...
void test_basic()
{
	...
	auto vv = sol.findLadders(begin, end, dict);
	for (auto v1: vv)
	{
		for (auto v2: v1)
		{
